...
#' the **Kuskokwim** figures (A–E), or a user-specified subset.
#'
#' - `"Ohlberger"` builds Figures **2, 3, 4, 5, 6, 7**.
#' - `"Kuskokwim"` builds Figures **A, B, C, D, E**.
...
#' @param ... Additional arguments passed to individual figure functions.
#'   For example, `summary_fn = "median"` can be used with Figures C and E
#'   to plot median values instead of means across iterations.
#'
#' @return Invisible named list of results for each figure generated.
#'
#' @examples
#' \dontrun{
#' # Generate all Kuskokwim figures with default settings (mean)
#' make_figures(figures = "Kuskokwim", data = my_run)
#'
#' # Generate Figures C and E using median instead of mean
#' make_figures(figures = c("C", "E"), data = my_run, summary_fn = "median")
#'
#' # Generate all Kuskokwim figures using median
#' make_figures(figures = "Kuskokwim", data = my_run, summary_fn = "median")
#' }
#'
#' @export
make_figures <- function(
  ...
  if (length(wanted) == 0L) {
    stop("No valid figures selected. Use 'Ohlberger', 'Kuskokwim', or specific codes like 'A','B','C','D','E','2','3','4','5','6','7'.")
  }
  ...
  # ---- dispatch map ------------------------------------------------------------
  # All functions accept (data = run_obj, output_dir = figure_dir, ...)
  builders <- list(
